...
\gdef\beginsong{%
  ...
  \startXmachine\relax}% do not eat first char
...
\long\def\firstEOL{%
  \ifx\nextEOL\EOLtok% end par
     ...
     \let\next=\startXmachine% skip EOLs and begin new one
...
{\catcode`\ =12%
\gdef\startXmachine{%
\let\Xaction=\skipEOLs\bgroup\catcode`\ =12\let\SPACE= \Xmachine}
}
%
\def\Xmachine#1{\futurelet\Xtok\Xbody}%
\def\Xbody{%
  \message{\meaning\Xtok}%
  \message{\the\savenum}%
  \Xaction{\Xtok}%
  \ifx\Xaction\ENDMACHINE\let\next=\relax
  \else\let\next=\Xmachine
  \fi\next}
%%%%  
\def\skipEOLs#1{\ifx#1\EOLtok\else\getverse{#1}\fi}%
\def\getverse#1{%
  \global\savenum={}%
  \ifx#1R\global\let\TypeOfVerse=R\let\Xaction=\getnum%
  \else\ifx#1M\global\let\TypeOfVerse=M\let\Xaction=\getnum%
  \else\global\let\TypeOfVerse=V\getnum{#1}%
  \fi\fi}
\def\getnum#1{%
  \let\Xaction=\relax
  \ifadddigit{#1}{\let\Xaction=\getnum}%
  \ifx:#1\xdef\NumOfVerse{\the\savenum}\let\Xaction=\skipspaces\fi%
  \ifx\Xaction\relax% Oh, it is no verse mark, return it
    \egroup% restore normal spaces
    \let\saveTOV=\TypeOfVerse%
    \global\let\TypeOfVerse=0\let\Xaction=\ENDMACHINE%
    \begintext% here must be corect \TypeOfVerse
    \ifx V\saveTOV\else \saveTOV\fi\the\savenum% return readed toks
  \fi}  
%
\def\skipspaces#1{\ifx#1\SPACE\else%
    \egroup% restore normal spaces
    \let\Xaction=\ENDMACHINE\begintext\fi}
%    
...
% \((<general exp>)\)?\(/?<bass>\)?
% \|\(<restriced exp>\)?\(/<bass>\)?
% <modifier> .. arbitrary text, but sensefull are: +,-,m,mi,maj,dim,sus,add
...
\def\readchord#1{%
  \global\expandafter\savechord\expandafter{\the\savechord#1}% save for export
...
\def\bCHbass#1{\ifx/#1 
  \else\setformatC\MakeSlash%
  \MakeBassRaise\hbox\bgroup\MakeBase{#1}%
  \let\CHegroupchar=\relax% take control over )
  \let\chordnext=\eCHbass\fi}
\def\eCHbass#1{\ifx)#1\egroup\egroup%
  \else\let\chordnext=\CHerrend\CHreadtone{#1}\fi}
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
\def\MakeSlash{/}
\def\MakeExpPlus{{\the\scriptscriptfont\fam \char`+}}
\def\MakeExpMinus{{\the\scriptscriptfont\fam -}}
\def\MakeBasePlus{{\the\scriptfont\fam +}}
\def\MakeBaseMinus{{\the\scriptfont\fam -}}
%% some kerns and raises
\def\MakeAfterExpKern{\kern-.1em}
\def\MakeBassRaise{\raise -.3ex}
