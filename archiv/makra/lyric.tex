...
\def\inputsong#1{%
  ...
  \egroup% end text/chord environment; backspace all h/vglue
%  \unpenalty\setbox0=\lastbox\unskip%
%  \unpenalty\setbox0=\lastbox\unskip%
%  \unpenalty\setbox0=\lastbox\unskip%
  \endgroup}%
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
\def\begintext{%
  ...
  \vrule width 0pt}% to eat possible space
\def\endtext{%
...
\def\getverse{%
  ...
  \ifx R\Xtok\global\let\TypeOfVerse=R% read next tok
  \else\ifx M\Xtok\global\let\TypeOfVerse=M% read next tok
  \else\global\let\TypeOfVerse=V\let\next=\getnumX% continue with current
  \fi\fi\next}%
%%%%  
...
\def\getnumX{%
  ...
  \ifx\next\relax% end of the number 
    ...
    \else% Oh, it is no verse mark, return it 
      \xdef\SAVEDTEXT{\ifx V\TypeOfVerse\else\TypeOfVerse\fi\the\savenum}%
      \global\let\TypeOfVerse=0%
      \let\next=\endnumX%
    \fi%  
  \fi\next}%
