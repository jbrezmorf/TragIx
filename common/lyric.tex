...
\font\ChordFontBass=cmssbx10 at 8pt
% \font\ChordFontText="TeX Gyre Heros Bold" at 8pt
% \font\ChordFontExp="TeX Gyre Heros Bold" at 5pt
% \font\ChordFontBass="TeX Gyre Heros Bold" at 8pt

\font\ChordFontExpSymb=cmsy5
\font\ChordSymb=cmmib10 at 7pt

% \font\SongNameFont=lmroman12-bold at 14pt
\font\SongNameFont="TeX Gyre Schola Bold" at 14pt

\font\AuthorFont="Latin Modern Roman Slanted" at 11pt
% \font\AuthorFont="TeX Gyre Schola Italic" at 11pt
\font\CategoryFont="Latin Modern Roman Slanted" at 9pt

\newif\ifshowsourceref
\newif\ifshowcategory

\font\TextFont="Latin Modern Roman" at 10pt
\font\TextFontSlanted="Latin Modern Roman Slanted" at 10pt
% \font\TextFont="TeX Gyre Schola" at 11pt
% \font\TextFontSlanted="TeX Gyre Schola Italic" at 11pt

% fix for modern font + slanted text + possibility to use chord within slanted text
\def\sl#1{\bgroup\leavevmode{\TextFontSlanted #1}\egroup}

\font\PageFont=lmroman12-bold at 11pt
\font\IndexFont=lmroman12-regular at 10pt
% \font\PageFont="TeX Gyre Schola Bold" at 11pt
% \font\IndexFont="TeX Gyre Schola" at 11pt
%
...
%% separation of only chords (used in melorecitative lines)
\def\chsep{\hfil}
%%%%%%%%%%%%%%%%%%%%%%
%% TEXT TYPESETTING
%%%%%%%%%%%%%%%%%%%%%%
%% how to make song header from parametres:
%% #1=name, #2=author, #3=author code, #4=source, #5=source code, #6=category
\def\MakeSongHeader#1#2#3#4#5#6{{%
  \global\advance\songnum by 1%
  \noindent\songanchor\SongNameFont\unspace#1% song name
  \ifshowsourceref\if:#5:\else\ (\unspace#5)\fi\fi% source ref
  \write\TOCfile{\string\tocline{#1}{\the\pageno}}% write to toc file
  \if:#2:\else% author
  \hfill\bgroup\hskip.3em{\AuthorFont(\unspace#2\/)}\hskip.3em\egroup
  \fi\par%
  \penalty 10000%
  \ifshowcategory\if:#6:\else% category subtitle
  {\noindent\hskip1em{\CategoryFont\unspace#6}\par}%
  \penalty 10000%
  \fi\fi%
}}
...
% \if V\TypeOfVerse \rm\hbox to \parindent {\NumOfVerse.\hfil}%
  \if V\TypeOfVerse \TextFont\hbox to \parindent {\NumOfVerse.\hfil}%
  % \else\if R\TypeOfVerse \rm\hbox to \parindent {R\NumOfVerse:\hfil}%
  % \else\if C\TypeOfVerse \it\hbox to \parindent {\hfil}%
  \else\if R\TypeOfVerse \TextFont\hbox to \parindent {R\NumOfVerse:\hfil}%
  \else\if C\TypeOfVerse \TextFontSlanted\hbox to \parindent {\hfil}%
  % \else\if M\TypeOfVerse\rm% 
  \else\if M\TypeOfVerse\TextFont% 
  % \else\rm\hbox{\hskip\parindent}%
  \else\TextFont\hbox{\hskip\parindent}%
  \fi\fi\fi\fi}
...
\def\makeindex{
  \bgroup
  \def\InnerMargin{1.1cm}
  \def\OuterMargin{1.1cm}
...
\def\MakeAbsolutePageNum#1#2{% #1=page number, #2=position (left/right)
  \vbox to 0pt{%
    \vskip\vPageSize% go to bottom of page
    \vskip-\BottomMargin% come back up by bottom margin
    \vskip-6.5ex% more space from bottom - was -2ex
    \hbox to \hsize{%
      \PageFont%
      \ifx L#2#1\hfill\else\hfill#1\fi% L=left aligned, else right aligned
    }%
    \vss% absorb all vertical space
  }%
}

% Create both page numbers with absolute positioning
\def\MakePagePair{%
  \global\advance\pagenum by 1%
  \count1=\pagenum% left page number
  \global\advance\pagenum by 1%
  \count2=\pagenum% right page number
  % Create absolutely positioned page numbers
  \global\setbox1=\MakeAbsolutePageNum{\the\count1}{L}% Left
  \global\setbox2=\MakeAbsolutePageNum{\the\count2}{R}% Right
}

% When \preservelayouttrue, invisible placeholders reserve the same vertical
% space as the old (non-absolute) page numbers.  This keeps songs on the
% same pages as in already-printed editions.  For a fresh edition set
% \preservelayoutfalse in head.tex so content can flow naturally.
\newif\ifpreservelayout
\preservelayouttrue  % safe default: preserve layout

\def\MakeInvisiblePagePlaceholder{\vbox{%
  \PageFont\vskip 2ex%
  \hbox to \hsize{\PageFont\phantom{999}\hfill}%
  }}

% Left/right page number areas: placeholder when preserving, empty otherwise
\def\MakePageNumLeft{\ifpreservelayout\MakeInvisiblePagePlaceholder\fi}
\def\MakePageNumRight{\ifpreservelayout\MakeInvisiblePagePlaceholder\fi}
% auxiliar rule
...
% Clickable index -> song hyperlinks (invisible in print).
% Uses pdfTeX-compatible primitives available in both pdfTeX and LuaTeX/pdfcsplain.
\def\songanchor{\pdfdest name{song.\the\songnum} fit\relax}%
\def\toclink#1{\pdfstartlink attr{/Border [0 0 0]} goto name{song.#1}\relax}%
\def\tocendlink{\pdfendlink\relax}%
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\def\setlyricoutput{%
  \PageLayout
  % Always shrink vsize so content doesn't overlap the absolutely-positioned
  % page numbers.  (\preservelayout only controls the invisible placeholders
  % that keep songs on the same pages as an already-printed edition.)
  \pagenum=99
  \setbox0=\MakePageNum
  \pagenum=0
...
\def\lyricoutput{\poisepages
  \message{\the\vsize}
  % Create absolute page numbers
  \MakePagePair
  %left page
  {\advance\hoffset by \OuterMargin
   \offinterlineskip
   % Content with invisible placeholder + absolute page number overlay
   \setbox3=\vbox{\copy1\vskip-\ht1\box5\MakePageNumLeft}
   \count0=\count1\shipout\box3}
  %right page 
  {\advance\hoffset by \InnerMargin
   \offinterlineskip
   % Content with invisible placeholder + absolute page number overlay
   \setbox4=\vbox{\copy2\vskip-\ht2\box6\MakePageNumRight}
   \count0=\count2\shipout\box4}
}
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% poisepages -
...
% For dealing with literal parentheses. 
% Use @[ and @] to insert literal parentheses in the text.
\catcode`\@=13
\def@{%
  \futurelet\nexttok\maybeLiteralParen
}
\def\maybeLiteralParen{%
  \ifx\nexttok[%
    \let\next=\insertLiteralLParen
  \else\ifx\nexttok]%
    \let\next=\insertLiteralRParen
  \else
    \let\next=\relax
  \fi\fi
  \next
}
\def\insertLiteralLParen[{(}
\def\insertLiteralRParen]{)}
% 

% for dealing with chords within quotes
\def\uv#1{\bgroup\leavevmode„#1“\egroup}

  
%% header
{\catcode`\(=13\catcode`\^^M=13
% getheader - reads 7 header fields, calls formatting macro.
% #6 (E:) is edition metadata consumed but not typeset.
% #7 (K:) is passed through for conditional display.
\gdef\getheader N:#1^^MA:#2^^MAC:#3^^MZ:#4^^MZC:#5^^ME:#6^^MK:#7^^M{%
   \MakeSongHeader{#1}{#2}{#3}{#4}{#5}{#7}%
   \beginsong}%
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%555
% Tracing -- set to non-zero values only when debugging TeX macro issues.
% WARNING: non-zero values generate multi-GB log files and slow builds ~10x.
\showboxbreadth=0
\showboxdepth=0
\tracingmacros=0
\tracingcommands=0
