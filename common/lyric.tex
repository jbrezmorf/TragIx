...
% Invisible placeholder to reserve space (same size as original page numbers)
% Oggy note - this is necessary to preserve the layout of the content,
% which we want, since we already have some printed copies with this layout.
% We would like songs to stay on the same pages, etc.
\def\MakeInvisiblePagePlaceholder{\vbox{%
  \PageFont\vskip 2ex%
  \hbox to \hsize{\PageFont\phantom{999}\hfill}% invisible placeholder
  }}

% Left page number - invisible placeholder
\def\MakePageNumLeft{\MakeInvisiblePagePlaceholder}

% Right page number - invisible placeholder  
\def\MakePageNumRight{\MakeInvisiblePagePlaceholder}
% auxiliar rule
...
\def\setlyricoutput{%
  \PageLayout
% page numbering - reserve space with invisible placeholder to preserve content layout
  \pagenum=99
  \setbox0=\MakePageNum% measure original page number size
  \pagenum=0
  \message{\the\vsize,\the\ht0}
  \advance\vsize by -\ht0% reserve space exactly as before
