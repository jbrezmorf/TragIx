...
\def\setlyricoutput{%
  \PageLayout
  \ifpreservelayout
    % Reserve space matching old page numbers to keep songs on the same pages
    \pagenum=99
    \setbox0=\MakePageNum
    \pagenum=0
...
% getheader - reads 6 header fields, call formating macro,
% #6 (E:) is edition metadata consumed but not typeset.
\gdef\getheader N:#1^^MA:#2^^MAC:#3^^MZ:#4^^MZC:#5^^ME:#6^^M{%
